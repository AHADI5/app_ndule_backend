...
//celle-ci permet de suprimer une chanson connaissant l'id
public static function delete_music($id){
        global $db;
        $query= "delete from music where id_music=:id_music";
        $stat = $db->prepare($query);
        $execution = $stat->execute([
            ":id_music"=> $id,
        ]);
        if ($execution){
            return true;
